...
/**
 * A Hikari wrapper for use with {@link DsConfig} which allows the SQL connection properties to be
 * specified more easily by chaining the properties using the builder pattern. Also sets the
 * connection properties that we use every time (cachePrepStmts, prepStmtCacheSize,
 * prepStmtCacheSqlLimit) to suitable defaults so that they do not have to be explicitly defined for
 * each new DataSource. Create an instance by calling "new" and passing a {@link DsConfig} object as
 * the sole parameter. After instantiating the DataSource, obtain a {@link Connection} to the
 * database for executing SQL statements by calling {@link #getConnection()}.
 *
 * @see DsConfig
 */
